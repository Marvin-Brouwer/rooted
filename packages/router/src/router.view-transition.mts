 ...
 *
 * Overlapping navigations are normal in a router: routes load through dynamic imports,
 * so a back/forward press while a chunk is still fetching starts a second transition before the first has finished.
 * The browser then skips the first one and rejects its `ready` promise with an `AbortError`. That's the documented outcome of a skip,
 * not a failure, so it gets caught here instead of surfacing as an uncaught rejection in the console.
 *
