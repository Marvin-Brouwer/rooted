...
import type { RouteMatchers } from './matchers.mts'
...
export function developmentNotFound(config: ResolvedConfig, matchers: () => RouteMatchers) {
	return (server: ViteDevServer): (() => void) => {
		/** 200 for a page the host really serves, 404 for one it only shells out. */
		function statusFor(pathname: string): 200 | 404 {
			const { isStatic, dynamicStatus } = matchers()
			return isStatic(pathname) ? 200 : dynamicStatus
		}
		...
		server.middlewares.use((request, response, next) => {
			...
			if (matchers().isRoute(target.pathname)) {
				const status = statusFor(target.pathname)
				// A 200 is just vite serving the page as it always would.
				if (status === 200) return next()
				return void respondWithShell(server, config, target.url, response, next, status)
			}
