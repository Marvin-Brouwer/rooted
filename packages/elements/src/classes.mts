/**
 * A single CSS class value. Falsy variants (`undefined` and `null`) are automatically filtered out when building a class list,
 * so conditional classes can be expressed without explicit guards.
 */
export type CssClass = string | undefined | null

/**
 * One or more CSS class values accepted by the `classes` prop of every HTML element created via `create()` or `append()`.
 *
 ...
 *
 * Keys have to be strings, so this doesn't work with a `CssModule` value that TypeScript thinks might be `undefined`.
 * Use the list form of {@link cssClasses} for those.
 *
 ...
 *
 * Designed to compose cleanly with {@link CssClasses}: falsy values are filtered out automatically,
 * so static and conditional classes can be mixed in a single array without branching. Called with just a class name it returns that name,
 * which is handy when the rest of the array is conditional.
 *
...
/**
 * Joins several class names into one {@link CssClass}, dropping the falsy ones. Returns `undefined` when nothing is left,
 * so the result disappears from a `classes` array instead of adding an empty string.
 *
 ...
 *
 * Watch out for one overlap: a leading `undefined` or `null` is read as the condition,
 * so `cssClasses(undefined, 'a')` is `undefined` and not `'a'`.
