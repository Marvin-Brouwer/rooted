...
/**
 * Everything {@link resizeObserver} takes. The `box` key is the browser's own
 * `ResizeObserverOptions`, passed straight through to `observe()`.
 */
 ...
 *
 * `ResizeObserver` takes its `box` on `observe()` rather than on the
 * constructor, so every target here is observed with the same box.
 *
