...
export type UpdateNotificationOptions = {
	/**
	 * What to render once there's an update. A single node or string, or an
	 * array of them. `undefined` and `null` entries are skipped.
	 */
	children?: ElementChildren
}

/**
 * Renders its children only once a new version is installed and waiting. Until
 * then it renders nothing, so it's safe to leave mounted in a layout.
 *
 * It adds no element of its own, so whatever you put inside lands straight in
 * the parent's layout.
 *
 * The children are built when the component mounts, not when the update
 * arrives. Keep them cheap.
 *
...
export const UpdateNotification = component<UpdateNotificationOptions>({
	...
	onMount({ options, append, signal }) {
		...
		onUpdateReady(() => { append(...nodes) }, { signal })
	},
})
