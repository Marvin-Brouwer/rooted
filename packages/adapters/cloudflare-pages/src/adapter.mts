 ...
 *
 * There is no catch-all rule. Cloudflare serves the nearest `404.html` with a real `404` for a request that matches no file,
 * and it doesn't support `404` as a `_redirects` status, so a `/*  /404.html  200` line is the only way to answer an unknown path and it answers it wrong.
 * Note that Pages only does this when a top-level `404.html` exists, which the adapter always writes;
 * without one it assumes you're deploying a single-page application.
 *
 * The rules are written without a trailing slash, so both `/recipe/42` and `/recipe/42/` serve the page.
 * `vite dev` still redirects the first to the second; Cloudflare doesn't.
 *
 * This adapter is for **static hosting only**. For server-side logic with Cloudflare Workers or Functions,
 * that requires a separate adapter not covered here.
 *
