...
export type FastifyAdapterOptions = {
	 ...
	 * Each file must export a default `async function(app)` that registers plugins or middleware on the Fastify instance.
 ...
 * The server uses `@fastify/static` to serve pre-rendered HTML files and registers explicit handlers for parameterized routes,
