...
export type ExpressAdapterOptions = {
	 ...
	 * Each file must export a default `function(app)` that registers middleware on the Express instance.
	 * Files are loaded in lexicographic order, so numeric prefixes (`01-auth.mts`, `02-proxy.mts`) control load order.
	 * Middleware runs before the rooted static-file and route handlers.
	 *
	 * The same files also run during `vite dev` and `vite preview`, on Vite's own port, so you don't need a second process to reach them.
	 * Dev loads the sources through Vite, preview runs the built `dist/middleware/*.mjs`.
 ...
 * The server uses `express.static` to serve pre-rendered HTML files and registers explicit handlers for parameterized routes,
 * which serve the `404.html` SPA shell so the browser-side router renders the correct content.
 * Express uses the same `:param` syntax as the rooted router, so patterns map directly.
 *
 * Users start the server with `node dist/server.mjs`. The `PORT` environment variable controls the port (default: 3000).
 *
 * Returns several plugins: the build-time adapter, the not-found handler that gives `vite dev` and `vite preview` the same 404s and redirects as the generated server,
 * and one that runs `middlewarePath` on Vite's own port. Vite flattens nested plugin arrays,
 * so it still goes straight into `plugins` as one entry.
 *
