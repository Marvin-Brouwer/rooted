...
describe('localizationSeo()', () => {
	...
	test('resolves the locale token through a parent route', () => {
		...
		// Act
		const links = provider('/nl-NL/contact/')

		// Assert
		expect(byHreflang(links, 'en-GB')?.path).toBe('/en-GB/contact/')
	})
	...
	test('returns undefined for paths without a localized route', () => {
		...
		// Act
		const knownPath = provider('/plain/')
		const unknownPath = provider('/unknown/')

		// Assert
		expect(knownPath).toBeUndefined()
	...
	test('the html transform sets the lang attribute per variant', () => {
		...
		// Act
		const dutch = transform(shell, '/nl-NL/about/')
		const english = transform(shell, '/en-GB/about/')

		// Assert
		expect(dutch).toContain('<html lang="nl-NL">')
	...
	test('the html transform replaces an existing lang attribute', () => {
		...
		// Act
		const result = transform(withLang, '/nl-NL/about/')

		// Assert
		expect(result).toContain('<html lang="nl-NL">')
	})
	...
	test('the html transform leaves non-localized paths alone', () => {
		...
		// Act
		const result = transform(shell, '/plain/')
