...
async function check(files: SourceFiles, options: DictionaryCheckOptions = {}): Promise<string[]> {
	...
	hooks.configResolved({ root, command: 'build', plugins: [] } as unknown as ResolvedConfig)
	hooks.buildStart.call(context)
...
describe('localizationDictionaryCheck()', () => {
	...
	test('notes dictionary keys it can\'t read from source', async () => {
		...
		// Assert
		expect(warnings).toEqual([
			'nl-NL: 1 entry has a key that isn\'t a string literal in nl-NL.mts, so it isn\'t checked.',
		])
	})
})
